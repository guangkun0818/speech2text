...
""" Scaling layers, modified from icefall required by zipformer. """
        ...
        if x <= self.pairs[0][0]:
            ...
            for i in range(1, len(self.pairs)):
                ...
                if x >= cur_x and x <= next_x:
                    return cur_y + (next_y - cur_y) * (x - cur_x) / (next_x -
                                                                     cur_x)
    ...
    def get_common_basis(self,
        ...
        if include_crossings:
            ...
            for i in range(len(x_vals) - 1):
                if (y_vals1[i] > y_vals2[i]) != (y_vals1[i + 1] >
                                                 y_vals2[i + 1]):
...
class ScheduledFloat(torch.nn.Module):
    ...
    def __float__(self):
        ...
        if (batch_count is None or not self.training or
...
class MaxEigLimiterFunction(torch.autograd.Function):
    ...
    def backward(ctx, x_grad, *args):
        ...
        x_extra_grad = (x_orig.grad * ctx.grad_scale * x_grad.norm() /
...
class BiasNormFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        scales = (torch.mean((x - bias)**2, dim=channel_dim, keepdim=True)**
                  -0.5) * log_scale.exp()
        ans = x * scales
...
class BiasNorm(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        return BiasNormFunction.apply(x, self.bias, log_scale, self.channel_dim,
...
class BalancerFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        ctx.config = (min_mean, max_mean, min_rms, max_rms, grad_scale,
                      channel_dim)
        return x

    @staticmethod
    def backward(ctx,
                 x_grad: Tensor) -> Tuple[Tensor, None, None, None, None, None]:
        (x,) = ctx.saved_tensors
        (min_mean, max_mean, min_rms, max_rms, grad_scale,
         channel_dim) = ctx.config
        ...
        try:
            with torch.enable_grad():
                with torch.cuda.amp.autocast(enabled=False):
                    ...
                    stddev = (uncentered_var -
                              (mean * mean)).clamp(min=1.0e-20).sqrt()
                    ...
                    loss_grad = x.grad
                    loss_grad_rms = ((loss_grad**2).mean(
                        dim=mean_dims, keepdim=True).sqrt().clamp(min=1.0e-20))
...
class Balancer(torch.nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if random.random() < prob:
            ...
            return BalancerFunction.apply(x, min_mean, max_mean, min_rms,
                                          max_rms, grad_scale, self.channel_dim)
        else:
            return _no_op(x)
            ...
            with torch.enable_grad():
                with torch.cuda.amp.autocast(enabled=False):
                    ...
                    if metric < float(w.whitening_limit):
                        ...
                        scale = float(
                            w.grad_scale) * (x_grad.to(torch.float32).norm() /
...
class Whiten(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        grad_scale = float(self.grad_scale)
        if not x.requires_grad or random.random(
        ) > self.prob or grad_scale == 0:
            return _no_op(x)
        else:
...
class LimitParamValue(torch.autograd.Function):
    ...
    def backward(ctx, x_grad: Tensor):
        ...
        # x more negative).
        x_grad *= torch.where(torch.logical_and(x_grad < 0, x > ctx.max), -1.0,
                              1.0)
...
class DoubleSwishFunction(torch.autograd.Function):
    ...
    def forward(ctx, x: Tensor) -> Tensor:
        ...
        if requires_grad:
            ...
            d_scaled = (deriv -
                        floor) * (255.0 /
                                  (ceil - floor)) + torch.rand_like(deriv)
    ...
    def forward(ctx, x: Tensor) -> Tensor:
        ...
        with torch.cuda.amp.autocast(enabled=False):
            with torch.enable_grad():
                ...
                d_scaled = (grad -
                            floor) * (255.0 /
                                      (ceil - floor)) + torch.rand_like(grad)
                if __name__ == "__main__":
                    # for self-testing only.
                    assert d_scaled.min() >= 0.0
                    assert d_scaled.max() < 256.0
...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if dropout_p != 0.0:
            ...
            dropout_mask = (1.0 / (1.0 - dropout_p)) * (torch.rand(
                *dropout_shape, device=x.device, dtype=x.dtype) > dropout_p)
        else:
            dropout_mask = None
