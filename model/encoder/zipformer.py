...
""" Zipformer impl, based on https://arxiv.org/pdf/2310.11230v4. Modified from 
...
from model.layer.scaling import (
...
from model.layer.subsampling import Conv2dSubsampling


@dataclasses.dataclass
class Zipformer2Config:
    """ Zipformer2 config interface """
    feature_dim: int = 80
    ...
    pos_dim: int = 192
    dropout: FloatLike = ScheduledFloat(
        (0.0, 0.3), (20000.0, 0.1))  # see code below for default
    warmup_batches: float = 4000.0
    causal: bool = False
    chunk_size: Tuple[int] = (-1)
    left_context_frames: Tuple[int] = (-1)
    for_ctc: bool = False  # For Zipformer + CTC setting.
    num_tokens: int = 1000  # Forlast projection of Zipformer + CTC setting.
...
class Zipformer2(nn.Module):
    ...
    def __init__(self, config: Zipformer2Config) -> None:
        super(Zipformer2, self).__init__()

        self._feature_dim = config.feature_dim
        self._encoder_embed = Conv2dSubsampling(
        ...
        def _to_tuple(x):
            ...
            if isinstance(x, int):
                x = (x,)
            if len(x) == 1:
                x = x * len(config.downsampling_factor)
            else:
                assert len(x) == len(config.downsampling_factor) and isinstance(
                    x[0], int)
            return x

        self.output_downsampling_factor = config.output_downsampling_factor  # int
        self.downsampling_factor = config.downsampling_factor  # tuple
        self.encoder_dim = config.encoder_dim = _to_tuple(
            config.encoder_dim)  # tuple
        self.encoder_unmasked_dim = config.encoder_unmasked_dim = _to_tuple(
            config.encoder_unmasked_dim)  # tuple
        num_encoder_layers = _to_tuple(config.num_encoder_layers)
        self.num_encoder_layers = config.num_encoder_layers
        self.query_head_dim = config.query_head_dim = _to_tuple(
            config.query_head_dim)
        self.value_head_dim = config.value_head_dim = _to_tuple(
            config.value_head_dim)
        pos_head_dim = _to_tuple(config.pos_head_dim)
        self.num_heads = config.num_heads = _to_tuple(config.num_heads)
        feedforward_dim = _to_tuple(config.feedforward_dim)
        self.cnn_module_kernel = config.cnn_module_kernel = _to_tuple(
            config.cnn_module_kernel)

        self.causal = config.causal
        self.chunk_size = config.chunk_size
        self.left_context_frames = config.left_context_frames

        for u, d in zip(config.encoder_unmasked_dim, config.encoder_dim):
            assert u <= d

        # each one will be Zipformer2Encoder or DownsampledZipformer2Encoder
        encoders = []

        num_encoders = len(config.downsampling_factor)
        for i in range(num_encoders):
            encoder_layer = Zipformer2EncoderLayer(
                embed_dim=self.encoder_dim[i],
                pos_dim=config.pos_dim,
                num_heads=self.num_heads[i],
                query_head_dim=self.query_head_dim[i],
                pos_head_dim=pos_head_dim[i],
                value_head_dim=self.value_head_dim[i],
                feedforward_dim=feedforward_dim[i],
                dropout=dropout,
                cnn_module_kernel=self.cnn_module_kernel[i],
            ...
            encoder = Zipformer2Encoder(
                ...
                pos_dim=config.pos_dim,
                dropout=dropout,
                warmup_begin=config.warmup_batches * (i + 1) /
                (num_encoders + 1),
                warmup_end=config.warmup_batches * (i + 2) / (num_encoders + 1),
                final_layerdrop_rate=0.035 *
                (config.downsampling_factor[i]**0.5),
            )

            if config.downsampling_factor[i] != 1:
                encoder = DownsampledZipformer2Encoder(
                    encoder,
                    dim=self.encoder_dim[i],
                    downsample=config.downsampling_factor[i],
                    dropout=dropout,
                )
        ...
        self.downsample_output = SimpleDownsample(
            max(self.encoder_dim),
            downsample=config.output_downsampling_factor,
            dropout=dropout)

        self._for_ctc = config.for_ctc
        if self._for_ctc:
            self._ctc_projection = nn.Linear(in_features=max(self.encoder_dim),
                                             out_features=config.num_tokens)
        else:
            self._ctc_projection = nn.Identity()
    ...
    def get_feature_masks(self, x: Tensor) -> Union[List[float], List[Tensor]]:
        ...
        mask1 = (torch.rand(1, batch_size, 1, device=x.device) >
    ...
    def get_chunk_info(self) -> Tuple[int, int]:
        ...
        if chunk_size == -1:
            ...
            if torch.jit.is_scripting() or torch.jit.is_tracing():
                assert len(
                    self.left_context_frames) == 1, self.left_context_frames
    ...
    def forward(
        ...
        """
        # Subsampling layer forward
        x, x_lens = self._encoder_embed(x, x_lens)
        ...
        for i, module in enumerate(self.encoders):
            ...
            x = module(
                ...
                src_key_padding_mask=(None if src_key_padding_mask is None else
    ...
    def streaming_forward(self,
        ...
        """ Simulated streaming forward for inference """
        self.causal = True
        ...
        pad_len = 30
        x_lens += pad_len
        x = F.pad(x, pad=(0, 0, 0, pad_len), value=math.log(1e-10))

        enc_out, enc_out_length = self.forward(x, x_lens)
        return enc_out, enc_out_length

    def _get_attn_mask(self, x: Tensor, chunk_size: int,
                       left_context_chunks: int) -> Optional[Tensor]:
        """
        ...
        if left_context_chunks >= 0:
            num_encoders = len(self.encoder_dim)
            assert all(chunk_size *
                       left_context_chunks >= (self.cnn_module_kernel[i] // 2) *
                       self.downsampling_factor[i] for i in range(num_encoders))
        else:
    ...
    def _encoder_layer_streaming_forward(
        self,
        x: Tensor,
        x_lens: Tensor,
        states: List[Tensor],
    ...
    @torch.jit.export
    def _get_encoder_layer_init_states(
            self,
            batch_size: int = 1,
            device: torch.device = torch.device("cpu"),
    ) -> List[Tensor]:
        """Get initial states of Encoder layers
        ...
        for i, module in enumerate(self.encoders):
            ...
            for layer in range(num_layers):
                cached_key = torch.zeros(downsample_left, batch_size,
                                         key_dim).to(device)
                cached_nonlin_attn = torch.zeros(
                    1, batch_size, downsample_left,
                    nonlin_attn_head_dim).to(device)
                cached_val1 = torch.zeros(downsample_left, batch_size,
                                          value_dim).to(device)
                cached_val2 = torch.zeros(downsample_left, batch_size,
                                          value_dim).to(device)
                cached_conv1 = torch.zeros(batch_size, embed_dim,
                                           conv_left_pad).to(device)
                cached_conv2 = torch.zeros(batch_size, embed_dim,
                                           conv_left_pad).to(device)
                states += [
                    cached_key,
                    cached_nonlin_attn,
    ...
    def get_init_states(
        ...
        """
        states = self._get_encoder_layer_init_states(batch_size, device)

        embed_states = self._encoder_embed.get_init_states(batch_size, device)

        states.append(embed_states)

        processed_lens = torch.zeros(batch_size,
                                     dtype=torch.int64,
                                     device=device)
    ...
    def streaming_step(
            self, x: Tensor,
            states: List[torch.Tensor]) -> Tuple[Tensor, List[torch.Tensor]]:
        """ Wrapped for onnx/libtorch inference """
        ...
        x, x_lens, new_cached_embed_left_pad = self._encoder_embed.streaming_forward(
            x=x,
            x_lens=x_lens,
        ...
        src_key_padding_mask = torch.zeros(N, chunk_size, dtype=torch.bool)
        ...
        src_key_padding_mask = torch.cat([processed_mask, src_key_padding_mask],
                                         dim=1)
        ...
        (
            ...
            encoder_out_lens,
            new_encoder_states,
        ) = self._encoder_layer_streaming_forward(
            x=x,
            x_lens=x_lens,
        ...
        # Now encoder_out is of shape (N, T, encoder_out_dim)
        ...
        # CTC compliance: (N, T, num_tokens)
        if self._for_ctc:
            encoder_out = F.log_softmax(self._ctc_projection(encoder_out),
                                        dim=-1)
...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        ff3_skip_rate: FloatLike = ScheduledFloat((0.0, 0.1), (4000.0, 0.01),
                                                  (50000.0, 0.0)),
        bypass_skip_rate: FloatLike = ScheduledFloat((0.0, 0.5), (4000.0, 0.02),
                                                     default=0),
    ) -> None:
    ...
    def get_sequence_dropout_mask(self, x: Tensor,
        ...
        if (dropout_rate == 0.0 or not self.training or
    ...
    def forward(
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            ...
            attention_skip_rate = (float(self.attention_skip_rate)
        ...
        src = src + (na if self_attn_dropout_mask is None else na *
                     self_attn_dropout_mask)

        self_attn = self.self_attn1(src, attn_weights)

        src = src + (self_attn if self_attn_dropout_mask is None else
                     self_attn * self_attn_dropout_mask)
    ...
    def _get_bypass_scale(self, batch_size: int):
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing(
            ...
            if straight_through_rate != 0.0:
                mask = (torch.rand(
                    (batch_size, 1), device=ans.device) < straight_through_rate)
...
class CompactRelPositionalEncoding(torch.nn.Module):
    ...
    def extend_pe(self, x: Tensor, left_context_len: int = 0) -> None:
        ...
        # is important.
        x_compressed = (compression_length * x.sign() *
...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def __init__(
        ...
        self.in_proj = ScaledLinear(embed_dim,
                                    in_proj_dim,
                                    bias=True,
    ...
    def forward(
        ...
        p = x[..., 2 * query_dim:]
        assert p.shape[-1] == num_heads * pos_head_dim, (p.shape[-1], num_heads,
                                                         pos_head_dim)
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=False):
                ...
                attn_weights_entropy = (
                    -((attn_weights + 1.0e-20).log() *
                      attn_weights).sum(dim=-1).mean(dim=(1, 2)))
                logging.info(
...
class NonlinAttention(nn.Module):
    ...
    def __init__(
        ...
        self.out_proj = ScaledLinear(hidden_channels,
                                     channels,
                                     bias=True,
                                     initial_scale=0.05)
...
class ConvolutionModule(nn.Module):
    ...
    def forward(
        ...
        if (not torch.jit.is_scripting() and not torch.jit.is_tracing() and
                chunk_size >= 0):
